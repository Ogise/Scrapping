...
def send_deal_to_discord(deal, channel):
    ...
    description = (
        ...
        f"**Price:** {deal['price']}\n"
        f"**Promo Discount:** {deal['promo_discount']}\n"
        f"**Promo Code:** {deal['promo_code']}\n"
        f"**Image:** {deal['image']}\n"
        f"[Click here to buy]({deal['product_link']})"
    )
    ...
    try:
        ...
        print(f"Deal successfully sent to {channel} channel: {deal['title']}")
...
if __name__ == "__main__":
    ...
    for fetch_deals in websites:
        deals = fetch_deals()
        if not deals:
            print(f"No deals found from {fetch_deals.__name__}")
        for deal in deals:
